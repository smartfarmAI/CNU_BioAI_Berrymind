...
class DeviceFSM:
    ...
    def _send_command(self, cmd_name: str, duration_sec: int) -> int:
        ...
        # 응답이 state_code를 포함할 때 실시간 반영
        if "state_code" in js and js.get("state_code") is not None:
            try:
                self.last_state_code = int(js.get("state_code"))
            except Exception:
                self.last_state_code = STATCODE["ERROR"]
        self.last_opid = opid
        return opid
    ...
    def on_start(self, opid: int, deadline_ts: float):
        ...
        print(f"[{self.actuator_name}] opid={opid} 시작되었습니다.")

    def on_finish(self):
        print(f"[{self.actuator_name}] opid={self.want_opid} 완료되었습니다.")
        self.want_opid = None # opid 초기화

    def on_fail(self):
        print(f"[{self.actuator_name}] opid={self.want_opid} 에러")
        self.want_opid = None
    ...
    async def _verify_loop(self):
        ...
        while True:
            ...
            try:
                ...
                # 상태 읽기 실패 > TTL 내 재시도, TTL 초과 시 fail, opid/state_code 파싱실패> error로 풀백
                if self.want_opid and time.time() > self.deadline_ts:
                    self.fail()
                    continue
            ...
            try:
                ...
                self.last_opid = None
            try:
                self.last_state_code = int(code) if code is not None else STATCODE["ERROR"]
            except Exception:
                self.last_state_code = STATCODE["ERROR"]
