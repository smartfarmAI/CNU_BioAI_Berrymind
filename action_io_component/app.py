...
@app.post("/actuators/{name}/command")
def post_command(name: str, body: CommandIn):
    ...
    if name in {"SKY_WINDOW_LEFT","SKY_WINDOW_RIGHT","SHADING_SCREEN","HEAT_CURTAIN"}:
        opid = act.send(Command(name=body.name, duration_sec=body.duration_sec or 0), target_pct=100)
    elif name == "NUTRIENT_PUMP":
        opid = act.send(Command(name=body.name, duration_sec=body.duration_sec or 0), ec=2.0, ph=6.0)
    else:
        opid = act.send(Command(name=body.name, duration_sec=body.duration_sec or 0))
    return {"opid": opid}
